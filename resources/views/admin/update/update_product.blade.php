  ...
  <div class="w-full max-w-md p-8 space-y-8 bg-blue-600 rounded-lg shadow-lg">
    ...
    <form class="mt-8 space-y-6" method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data">
      ...
      <div class="space-y-4">
        ...
        <div>
          <label for="foto_produk" class="block mb-2 text-sm font-medium text-white">Foto Produk</label>
          <input id="foto_produk" name="foto_produk" type="file" class="relative block w-full px-3 py-2 text-white placeholder-white bg-blue-500 border border-blue-500 rounded-md appearance-none focus:outline-none focus:ring-blue-300 focus:border-blue-300 sm:text-sm">
          @if ($product->gambar_produk)
          <div class="mt-4">
            <img src="{{ $product->gambar_produk }}" alt="Current Product Image" class="object-cover w-32 h-32 rounded-md">
          </div>
          @endif
        </div>
      </div>
